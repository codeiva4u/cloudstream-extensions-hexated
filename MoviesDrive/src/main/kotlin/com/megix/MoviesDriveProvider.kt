...
class MoviesDriveProvider : MainAPI() {
    ...
    private fun Element.toSearchResult(): SearchResponse {
        ...
        val title = imgElement?.attr("title")
        val trimTitle = title?.replace("Download ", "") ?: ""

        val href = fixUrl(this.selectFirst("figure > a")?.attr("href").toString())
        val posterUrl = fixUrlNull(imgElement?.attr("src").toString())
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val trimTitle = ogTitle?.replace("Download ", "") ?: ""
                ...
                buttons.forEach { button ->
                    ...
                    elements.forEach { element ->
                        ...
                        if (element.tagName() == "span") {
                            ...
                            while (linkTag != null && linkTag.text()
                                    .contains("HubCloud", ignoreCase = true)
                            ) {
    ...
    override suspend fun loadLinks(
        ...
        when {
            data.contains("graph.") -> {
                ...
                val links = regex.findAll(data).map { it.value }.toList()
                links.amap {
            ...
            data.contains("moviesdrive") -> {
                ...
                val hubCloudLinks = hubCloudRegex.findAll(data).map { it.value }.toList()
